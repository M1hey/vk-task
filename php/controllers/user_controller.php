...
function process_user($user) {
    ...
    include_full_page(get_user_prepared_view_name_by_type($user, $user['account_type']));
}
...
function show_user_ajax($user) {
    set_user_view_vars($user);
    include_only_content(get_user_prepared_view_name_by_type($user, $user['account_type']));
}

function get_user_prepared_view_name_by_type($user, $type) {
    switch ($type) {
